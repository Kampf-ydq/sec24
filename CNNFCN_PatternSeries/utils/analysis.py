...
class Analyzer(object):
    ...
    def generate_classification_report(self, digits=4, number_of_thieves=2, maxcharlength=35):
        """
        ...
        headers = ["precision", "recall", "f1-score", "f1/4-score", "rel. freq.", "abs. freq.", "biggest thieves"]
        ...
        for i in sorted_class_indices:
            ...
            for v in (self.precision[i], self.recall[i], self.f1[i], self.f1_4[i],
        ...
        for v in (np.average(self.precision, weights=relative_freq),
                  ...
                  np.average(self.f1_4, weights=relative_freq)):
            values += ["{0:0.{1}f}".format(v, digits)]
    ...
    def analyze_classification(self, y_pred, y_true, class_names, excluded_classes=None):
        ...
        # 20231003
        self.f1_4 = (17 * self.precision * self.recall) / (self.precision + 16 * self.recall)
        ...
        self.f1_avg = 2 * (self.prec_avg * self.rec_avg) / (self.prec_avg + self.rec_avg)
        if excluded_classes:
            print(
        ...
        return {"total_accuracy": self.total_accuracy, "precision": self.precision, "recall": self.recall,
                "f1": self.f1, "f1/4": self.f1_4, "support": self.support, "prec_avg": self.prec_avg, "rec_avg": self.rec_avg, "f1_avg": self.f1_avg}
