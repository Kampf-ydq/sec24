...
def inverse_transformed_samples(confscore, p):
    ...
    Parameters
    ----------
        confscore: the max confidence score sets
        p: the classification accuracy of the classifier
    Returns
    -------
    parameter theta.
    """
    # Using kernel density estimation
    kde = gaussian_kde(confscore)
    # Generating random samples on probability density functions
    samples = kde.resample(size=len(confscore))
    # Applying inverse transformations to samples (in percentiles)
    theta = np.percentile(samples, 1 - p, axis=1)
    # print(theta)
    return theta
