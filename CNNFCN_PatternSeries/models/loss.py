...
def get_loss_module(config):
    ...
    if task == "classification":
        ...
        # return ClassAndReconstLoss(reduction='none')
    if task == "regression":
...
class ClassAndReconstLoss(nn.Module):
    def __init__(self, reduction: str = 'mean'):
        super().__init__()
        self.reduction = reduction
        self.mse_loss = nn.MSELoss(reduction='mean')
        self.crs_loss = NoFussCrossEntropyLoss(reduction='none')

    def forward(self, pred, target, recon, x):
        crsloss = self.crs_loss(pred, target)
