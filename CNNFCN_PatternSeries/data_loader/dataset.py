...
class ClassiregressionDataset(Dataset):
    ...
    def __init__(self, data, indices, binary_classify=False, pos=1):
        super(ClassiregressionDataset, self).__init__()

        self.data = data  # this is a subclass of the BaseData class in data.py
        self.IDs = indices  # list of data IDs, but also mapping between integer index and ID
        self.feature_df = self.data.feature_df.loc[self.IDs]
        self.feature_mat = self.data.feature_mat.loc[self.IDs]

        if binary_classify:  # Used to set binary classification labels when training the discriminator.
            labels = []
            for i in self.IDs:
                if i <= (pos - 1):  # pos: Index of known and unknown class divisions
                    labels.append(0)  # known
                else:
                    labels.append(1)  # unknown
            self.labels_df = pd.DataFrame(labels,
