...
import sktime.datasets._data_io as load_data

from CNNFCN_PatternSeries.data_loader import utils
...
class TSRegressionArchive(BaseData):
    ...
    def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, limit_size=None, config=None):
        ...
        self.all_df, self.labels_df, self.feature_mat = self.load_all(root_dir, file_list=file_list, pattern=pattern)
    ...
    def load_all(self, root_dir, file_list=None, pattern=None):
        ...
        all_df, labels_df, origdf = self.load_single(input_paths[0])  # a single file contains dataset

        return all_df, labels_df, origdf
    ...
    def load_single(self, filepath):
        ...
        origdf = df  # Returning raw DataFrame for other purposes
